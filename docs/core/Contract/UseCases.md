...
- **失败结果**：返回 `ConfigLoadFailed` 或 `ConfigInvalid`，保留 Home。
- **状态转换**：Home → EditMode；失败时保持 Home。
- **完成边界**：EditMode 已提交。
- **异步后续**：无。
...
- **失败结果**：按处理阶段返回 `ConfigLoadFailed`、`ConfigInvalid` 或 `PlanBuildFailed`，保留 Home。
- **状态转换**：Home → ProductionMode；失败时保持 Home。
...
- **处理过程**：
  ...
  3. 如果检查正在运行，则向 Inspection Actor 请求后台取消。
- **成功结果**：进入 Home，不等待取消结束。
- **失败结果**：无业务失败；内部取消通信失效时直接 `panic`。
- **状态转换**：EditMode 或 ProductionMode → Home。
- **完成边界**：Home 已提交，模式资源已释放，必要的取消请求已发出。
- **异步后续**：Inspection Actor 可能继续执行取消流程。
- **资源变化**：原模式资源被释放；任务资源继续由 Inspection Actor 持有，直至任务终止。
- **并发关系**：检查事件和返回主页命令按 App Controller 的串行处理顺序裁决。Home 提交后处理的旧会话事件不得更新展示状态。
...
- **成功结果**：内存草稿包含新字段值。
- **失败结果**：返回字段校验失败原因，原草稿保持不变。
- **状态转换**：保持 EditMode，仅更新其 Draft Config。
...
- **失败结果**：按处理阶段返回 `ConfigInvalid`、`PlanBuildFailed` 或 `ConfigSaveFailed`。
- **状态转换**：保持 EditMode；成功时更新 Draft Config 的 `revision`。
...
- **状态转换**：AppState 保持 EditMode；Inspection Actor 从 `Idle` 转入 `Running`。
- **完成边界**：Inspection Actor 已进入 `Running`，且 Worker 已接受任务。
...
- **状态转换**：AppState 保持 ProductionMode；Inspection Actor 从 `Idle` 转入 `Running`。
- **完成边界**：Inspection Actor 已进入 `Running`，且 Worker 已接受任务。
- **异步后续**：发布最新检查状态；后续产生检查完成、检查失败或检查超时事件。
...
- **触发方**：应用运行环境或 GUI。
- **接收方**：App Controller。
- **前置状态**：ApplicationLifecycle 为 `Running`；AppState 可以为任意模式。
- **输入**：无。
- **处理过程**：
  1. App Controller 将 ApplicationLifecycle 原子地切换为 `ShuttingDown`；
  2. 拒绝队列中排在关机命令之后及之后新到达的应用命令；
  3. 请求 Camera Actor 停止采集并关闭摄像头，等待其确认停止；
  4. 如果检查正在运行，则请求 Inspection Actor 取消并等待其回到 `Idle`；
  5. 关闭 Inspection Worker 和各 Actor；
  6. 释放 AppState、Latest Frame Store 及其他应用资源；
  7. 将 ApplicationLifecycle 切换为 `Terminated`。
- **成功结果**：应用全部组件已停止，运行期资源已释放。
- **失败结果**：无业务失败；组件关闭失败、内部通信失效或取消宽限期耗尽时直接 `panic`。
- **状态转换**：ApplicationLifecycle 从 `Running` 经 `ShuttingDown` 转入 `Terminated`。`ShuttingDown` 不属于 AppState。
- **完成边界**：ApplicationLifecycle 已进入 `Terminated`。
- **异步后续**：无。
- **资源变化**：按处理过程释放全部应用资源。当前 AppState 不必先转换为 Home。
- **并发关系**：关机命令之前的命令先完成；关机命令之后的命令返回 `ShuttingDown`。关机开始后不可撤销。

关机与检查完成或失败的竞争由 Inspection Actor 的串行处理顺序裁决：

- Actor 先处理完成或失败输出时，任务正常结束，随后关机流程确认其为 `Idle`；
- Actor 先处理关机取消请求时，进入 `Cancelling`，之后到达的完成或失败输出一律丢弃；
- 因关机取消的检查不生成正常结果、失败展示或面向 GUI 的超时结果；
- Worker 停止后，Inspection Actor 释放任务资源、回到 `Idle`，并向关机流程确认取消完成。
